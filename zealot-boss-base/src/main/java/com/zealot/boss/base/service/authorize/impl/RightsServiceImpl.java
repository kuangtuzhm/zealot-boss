...
public class RightsServiceImpl implements RightsService {
	...
	public void updateRight(Rights right) throws AppException {
		...
		// String[] ignoreArray = { "rightCode", "createTime",
		// "state","createdBy","parentCode","sysCode","type", };
	...
	private List<Rights> queryChilderenRight(String parentCode)
			throws AppException {
	...
	public List<SimpleRights> querySimpleRightList(Integer state)
			throws AppException {
	...
	public void setRoleRights(List<SimpleRights> allRights,
			List<Rights> roleRights) {
		Map<String, Integer> map = new HashMap<String, Integer>();
		if (roleRights != null && roleRights.size() > 0 && allRights != null
				&& allRights.size() > 0) {
	...
	public void updateRoleRights(Integer roleId, List<String> rightCodes)
			throws AppException {
		roleRightDao.deleteByRole(roleId);

		RoleRight rr = null;
		for (String rightCode : rightCodes) {
			rr = new RoleRight();
			rr.setRightCode(rightCode);
			rr.setRoleId(roleId);
			roleRightDao.saveRoleRight(rr);
		}
		authorizationService.clearRightRoles();
	}
	...
	private List<SimpleRights> getChildren(SimpleRights node,
			List<SimpleRights> allRights) {
